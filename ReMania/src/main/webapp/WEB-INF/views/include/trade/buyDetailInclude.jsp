...
<div class="container">
	<div class="panel-group">
					...
					<c:choose>
						...
						<c:otherwise>
							<button type="button" class="btn btn-default"
								id="contentDisplayOn">내용보기</button>
							<button type="button" class="btn btn-default"
								id="contentDisplayOff">내용감추기</button>
						</c:otherwise>
					</c:choose>
				</center>
			</div>
		...
		<div id="trade_content">
			...
			<pre
				style="width: 95%; height: 30em; margin: 0 auto; background-color: white; color: black; border: 0px solid white;">${buyDTO.content}</pre>
		</div>
	</div>
	<c:if test="${tradeDTO == null}">
		<br />
		...
		<p style="text-align: left;">
			<button type="button" class="btn btn-danger"
				onclick="buyUpdate('${login.email}', '${buyDTO.email}', '${buyDTO.buynum}','${buyPageDTO.curPage}','${category}', '${sort}', '${searchType}', '${searchValue}')">수정하기</button>
			<button type="button" class="btn btn-danger"
				onclick="buyDelete('${login.email}', '${buyDTO.email}', '${buyDTO.buynum}','${buyPageDTO.curPage}','${category}', '${sort}', '${searchType}', '${searchValue}')">삭제하기</button>
		</p>
				...
				<div class="panel panel-danger">
					...
					<div class="panel-body">
						<form id="comment" action="comment" method="post">
							<input type="hidden" name="call" value="comment" /> <input
								type="hidden" name="category" value="buy" /> <input
								type="hidden" name="curPage" value="${buyPageDTO.curPage}" /> <input
								type="hidden" name="boardnum" value="${buyDTO.buynum}" /> <input
								type="hidden" name="type" value="${searchType}" /> <input
								type="hidden" name="value" value="${searchValue}" /> <font
								color="black"> <textarea id="usercomment"
									name="usercomment" rows="5" cols="100"
									style="border-style: none; outline: none; resize: none;"
									placeholder="여러분들의 소중한 의견을 작성해주세요."></textarea>
							</font>
						</form>
					</div>
				</div>
			...
			<c:forEach var="comment" items="${commentList}" varStatus="status">
				<c:choose>
					<c:when test="${comment.comment_level != 0}">
						<div
							style="float: left; width: 7.5%; margin-left: ${(comment.comment_level-1)*7.5}%;">
							<br />
							<br />
							<br /> <img src="images/answerArrow.png" width="30" />
						</div>
						<div class="panel panel-default"
							style="width: ${100-(comment.comment_level*7.5)}%; margin-left: ${comment.comment_level*7.5}%;">
					</c:when>
					<c:otherwise>
						<div class="panel panel-default">
					</c:otherwise>
				...
				<div class="panel-heading">
					<small>${comment.author} (${comment.writeday})</small>
				</div>
				<div class="panel-body">
					<form action="updateComment" method="post">
						<input type="hidden" name="num" value="${comment.num}" /> <input
							type="hidden" name="category" value="${comment.category}" /> <input
							type="hidden" name="boardnum" value="${comment.boardnum}" /> <input
							type="hidden" name="curPage" value="${buyPageDTO.curPage}" /> <input
							type="hidden" name="type" value="${searchType}" /> <input
							type="hidden" name="value" value="${searchValue}" />
						<textarea name="usercomment"
							style="width: 80%; height: 5em; color: black; border-style: none; outline: none; float: left; resize: none; background-color: white;"
							disabled>${comment.usercomment}</textarea>
						<c:if test="${comment.email eq login.email}">
							<button type="button" class="btn btn-danger deleteComment"
								style="float: right; margin-bottom: 1px;">삭제하기</button>
							<button type="button" class="btn btn-danger commentUpdate"
								style="float: right; margin-top: 1px;">수정하기</button>
						</c:if>
					</form>
				</div>
		</div>
		<form name="recomment" action="comment" method="post">
			<input type="hidden" name="call" value="recomment" /> <input
				type="hidden" name="curPage" value="${buyPageDTO.curPage}" /> <input
				type="hidden" name="category" value="${comment.category}" /> <input
				type="hidden" name="parentComment" value="${comment.parentComment}" />
			<input type="hidden" name="boardnum" value="${comment.boardnum}" /> <input
				type="hidden" name="ref" value="${comment.ref}" /> <input
				type="hidden" name="step" value="${comment.step +1}" /> <input
				type="hidden" name="comment_level"
				value="${comment.comment_level +1}" /> <input type="hidden"
				name="type" value="${searchType}" /> <input type="hidden"
				name="value" value="${searchValue}" />
			<c:if test="${comment.comment_level <2}">
				<span style="text-align: right; float: none;"> <textarea
						type="text" name="usercomment" class="recomment"
						style="float: left; margin-top: 0.5em; border-style: none; border-bottom: 1px solid lightgray; width:${100-(comment.comment_level+25)}%; height: 2.5em; outline: none; resize: none; overflow: hidden; margin-left: ${comment.comment_level*7.5}%;"
						placeholder="re: "></textarea>
					<button type="button" class="btn btn-danger" style="float: right;"
						onclick="reComment(${status.index});">답변하기</button>
				</span>
			</c:if>
		</form>
		<br />
